...
@php
    $user = auth()->user();
    $canMostrador = $user->tienePermiso('BIB_MOSTRADOR_VER') || $user->tienePermiso('BIB_PRESTAMOS_CREAR') || $user->tienePermiso('BIB_PRESTAMOS_DEVOLVER');
    $canSolicitudes = $user->tienePermiso('BIB_SOLICITUDES_PANEL_VER') || $user->tienePermiso('BIB_SOLICITUDES_GESTIONAR');
    $canMultas = $user->tienePermiso('BIB_MULTAS_PANEL_VER') || $user->tienePermiso('BIB_MULTAS_VER');
    $canConsultas = $user->tienePermiso('BIB_CONSULTAS_PANEL_VER') || $user->tienePermiso('BIB_CONSULTA_VER') || $user->tienePermiso('BIB_REPORTES_VER');
@endphp
<style>
    .bib-operation-wrap {
        max-width: 1180px;
...
<div class="container-fluid">
    <div class="bib-operation-wrap">
        ...
        <div class="row">
            ...
            @if(! $canMostrador && ! $canSolicitudes && ! $canMultas && ! $canConsultas)
                <div class="col-12">
                    <div class="card">No tienes opciones operativas asignadas para Biblioteca.</div>
                </div>
            @endif
